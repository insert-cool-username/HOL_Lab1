...
class GFLocalization(Localization,GaussianFilter):
    ...
    def Localize(self, xk_1, Pk_1):
        ...
        if zk is None:
            zk=np.array({})
            self.Log(self.robot.xsk,xk_bar,Pk_bar,xk_bar,zk)
            # self.Log(self.robot.xsk,xk_bar[0:3],Pk_bar[:3, :3],xk_bar[0:3],zk)
            return xk_bar, Pk_bar
        else:
            xk, Pk = self.Update(zk, Rk, xk_bar, Pk_bar,Hk,Vk)
            self.Log(self.robot.xsk,xk,Pk,xk_bar,zk)
            # self.Log(self.robot.xsk,xk[0:3],Pk[:3, :3],xk_bar[0:3],zk)
            return xk, Pk # Original xk, Pk
