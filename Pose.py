...
class Pose3D(Pose):
    ...
    def oplus(AxB, BxC):
        ...
        # To convert it to (1,3) if it was given as (3,1), we use squeeze
        AxB = AxB.squeeze()
        BxC = BxC.squeeze()
        AxC = np.array([[AxB[0] + BxC[0]*cos(AxB[2]) - BxC[1]*sin(AxB[2]), AxB[1] + BxC[0]*sin(AxB[2]) + BxC[1]*cos(AxB[2]), AxB[2] + BxC[2]]])
        # At this point AxC.shape = (1,3)
        AxC=AxC.reshape(3, 1)
        AxC=Pose3D(AxC)
        return AxC
    ...
    def ominus(AxB):
        ...
        # To convert it to (1,3) if it was given as (3,1), we use squeeze
        AxB = AxB.squeeze()
        BxA = np.array([[-1*AxB[0]*cos(AxB[2]) - AxB[1]*sin(AxB[2]), AxB[0]*sin(AxB[2]) - AxB[1]*cos(AxB[2]), -1*AxB[2]]])
        # At this point BxA.shape = (1,3)
        return Pose3D(BxA.reshape(3, 1))
