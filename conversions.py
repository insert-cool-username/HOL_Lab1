...
def p2c(p):  # polar to cartesian conversion
    ...
    return f.CartesianFeature(np.array([[x, y]]).T)
...
def J_p2c(p):  # polar to cartesian conversion
    ...
    rho = p[0,0]
    theta = p[1,0]
    J = np.array([[cos(theta), -rho*sin(theta)],
                  [sin(theta), rho*cos(theta)]])
    return J


def c2p(c):  # cartesian to polar conversion
    """
    ...
    x = c[0,0]
    y = c[1,0]
    rho, theta = sqrt(x**2 + y**2), atan2(y,x)
    return f.PolarFeature(np.array([[rho, theta]]).T)


def J_c2p(c):  # cartesian to polar conversion
    """
    ...
    x = c[0,0]
    y = c[1,0]
    J = np.array([[x/(sqrt(x**2 + y**2)), y/(sqrt(x**2 + y**2))],
                  [-y/(x**2 + y**2), x/(x**2 + y**2)]])
    return J
