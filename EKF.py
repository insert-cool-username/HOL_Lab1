...
class EKF(GaussianFilter):
    ...
    def Update(self, zk, Rk, xk_bar, Pk_bar, Hk, Vk):
        ...
        K_gain = Pk_bar @ Hk.T @ np.linalg.inv(Hk @ Pk_bar @ Hk.T + Vk @ Rk @ Vk.T) # Inverse, check if its a squared matrix
        self.xk = xk_bar + K_gain @ (zk - self.h(xk_bar))
        I = np.identity((K_gain@Hk).shape[0])
